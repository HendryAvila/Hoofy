#!/usr/bin/env bash
# Hoofy Installer
# One-liner: curl -sSL https://raw.githubusercontent.com/HendryAvila/Hoofy/main/install.sh | bash
#
# This script:
...
detect_arch() {
    ...
    case "$arch" in
        ...
        *)
            ...
            error "Hoofy supports x86_64 (amd64) and arm64 only."
            exit 1
            ;;
    esac
}
...
get_latest_version() {
    ...
    if command -v curl &>/dev/null; then
        version=$(curl -sSL "https://api.github.com/repos/HendryAvila/Hoofy/releases/latest" \
            -H "Accept: application/vnd.github.v3+json" \
            | grep '"tag_name"' | head -1 | sed 's/.*"tag_name": *"v\{0,1\}\([^"]*\)".*/\1/')
    elif command -v wget &>/dev/null; then
        version=$(wget -qO- "https://api.github.com/repos/HendryAvila/Hoofy/releases/latest" \
            | grep '"tag_name"' | head -1 | sed 's/.*"tag_name": *"v\{0,1\}\([^"]*\)".*/\1/')
    else
    ...
    if [ -z "$version" ]; then
        ...
        error "https://github.com/HendryAvila/Hoofy/releases"
        exit 1
    fi
...
download_binary() {
    ...
    local ext="tar.gz"
    local archive_name="hoofy_${version}_${os}_${arch}.${ext}"
    local url="https://github.com/HendryAvila/Hoofy/releases/download/v${version}/${archive_name}"

    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap "rm -rf '$tmp_dir'" EXIT

    info "Downloading hoofy v${version} for ${os}/${arch}..."
    printf "  ${DIM}%s${NC}\n" "$url"
    ...
    if command -v curl &>/dev/null; then
        if ! curl -sSL --fail -o "${tmp_dir}/${archive_name}" "$url"; then
            ...
            error "Check available downloads: https://github.com/HendryAvila/Hoofy/releases/tag/v${version}"
            exit 1
        fi
    else
    ...
    local binary_path
    binary_path=$(find "${tmp_dir}" -name "hoofy" -type f | head -1)

    if [ -z "$binary_path" ]; then
        error "Could not find hoofy binary in the downloaded archive."
        exit 1
    fi
    ...
    if [ "$install_dir" = "/usr/local/bin" ] && [ ! -w "$install_dir" ]; then
        ...
        sudo mv "$binary_path" "${install_dir}/hoofy"
    else
        mkdir -p "$install_dir"
        mv "$binary_path" "${install_dir}/hoofy"
    fi

    success "Installed hoofy to ${install_dir}/hoofy"
}
...
verify_install() {
    local install_dir="$1"
    local binary="${install_dir}/hoofy"
    ...
    if echo "$version_output" | grep -q "hoofy"; then
...
main() {
    ...
    printf "  ${BOLD}${GREEN}║                                           ║${NC}\n"
    printf "  ${BOLD}${GREEN}║   🎉 Hoofy installed successfully!    ║${NC}\n"
    printf "  ${BOLD}${GREEN}║                                           ║${NC}\n"
    printf "  ${BOLD}${GREEN}╚═══════════════════════════════════════════╝${NC}\n"
    printf "\n"
    printf "  ${BOLD}Next: Add Hoofy to your AI tool's MCP config:${NC}\n\n"
    ...
    printf "    3. Describe your idea — Hoofy will guide you\n"
    printf "\n"
    printf "  ${BOLD}Useful commands:${NC}\n\n"
    printf "    ${CYAN}hoofy serve${NC}     Start the MCP server\n"
    printf "    ${CYAN}hoofy update${NC}    Update to the latest version\n"
    printf "    ${CYAN}hoofy --help${NC}    Show help\n"
    printf "\n"
    printf "  ${DIM}Docs: https://github.com/HendryAvila/Hoofy${NC}\n"
